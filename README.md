# Implement-reliable-communication-over-an-unreliable-link
...
solution.  **You should not make changes to any file other than `hw4.py`.**
...
Your solution / `hw4.py` file will be tested against stock versions of all the
other files in the repo, so any changes you make will not be present at
grading time.

Your solution must be contained in the `send` and `recv` functions in `hw4.py`.
...
For example, to see how your solution performs when transmitting a text file (e.g., test_data.txt),
 ...
 * A key part of this homework is determining how long to wait before resending
   ...
   (deviation of RTT * 4)".  You should check with your book and watch the corresponding lecture video for more details.
 ...
 * Keep your packets smaller than or equal to `homework4.MAX_PACKET` (1400
   bytes).
...
| Successful transmission, good throughput         |             2          |
...
Your solution will be graded out of 12 possible points.
